...
ArrayLike = _Union[Array, npt.ArrayLike]
...
def get_native_namespace_name(xp: ModuleType) -> str:
    ...
    name = xp.__name__
    return (name[len(f"{_compat_module_name()}."):] if name.startswith(f"{_compat_module_name()}.") else name)
